#!/usr/bin/env bash
# Keeneland Cup 2026: phrase-synced subs, 100 scenes, varied motion, 720p
set -euo pipefail
...
ASS_ESC=$(python3 -c "import pathlib; print(pathlib.Path('subs/keeneland.ass').resolve().as_posix().replace(':','\\\\:'))")
...
# First encode HQ then compress under 100MB if needed
ffmpeg -y -i "$TMP/video_silent.mp4" -i audio/narration.mp3 \
  ...
  out/keeneland_cup_2026.mp4

SIZE=$(stat -c%s out/keeneland_cup_2026.mp4)
echo "size_bytes $SIZE"
if [ "$SIZE" -gt 99000000 ]; then
  echo "re-encode under 100MB"
  ffmpeg -y -i out/keeneland_cup_2026.mp4 \
    -c:v libx264 -profile:v main -level 3.1 -pix_fmt yuv420p -preset slow -crf 22 \
    -c:a aac -b:a 144k -ar 44100 -ac 2 -movflags +faststart \
    out/keeneland_cup_2026_repo.mp4
  mv out/keeneland_cup_2026.mp4 out/keeneland_cup_2026_hq.mp4
  mv out/keeneland_cup_2026_repo.mp4 out/keeneland_cup_2026.mp4
fi

ffprobe -v error -show_entries format=duration,size -show_entries stream=codec_name,width,height,sample_rate,channels -of default out/keeneland_cup_2026.mp4
ls -lh out/keeneland_cup_2026*.mp4
